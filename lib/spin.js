...
var Spinner = function(config) {
    ...
    this.segment_angle = config.segment_angle || 30;
    ...
    this.segment_depth = config.segment_depth || 2;
...
Spinner.prototype = {
    ...
    _draw_cone: function() {
        ...
        var alpha = 0;
        ctx.save();
        for (; i < 361; i += total_segment_angle) {
    ...
    clear: function() {
        ...
        ctx.restore();
        ctx.arc(0, 0, this.max_radius - 1, 0, Math.PI * 2);
        ctx.fillStyle = this._rgba(this.background, 1);
        ctx.fill();
